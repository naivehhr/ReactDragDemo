...
const widgetMap = {
  boolean: {
    ...
    select: "SelectWidget",
    hidden: "HiddenWidget",
    antdRadio: "AntdRadioWidget",
    antdCheckbox: "AntdCheckboxWidget",
    antdCheckboxes: "AntdCheckboxesWidget",
    antdDisplay: "AntdDisplayWidget",
  },
  string: {
    text: "TextWidget",
    ...
    color: "ColorWidget",
    file: "FileWidget",
    antdText: "AntdTextWidget",
    antdSelect: "AntdSelectWidget",
    antdDisplay: "AntdDisplayWidget",
    antdDateWidget: "AntdDateWidget",
    antdFileWidget: "AntdFileWidget",
    antdGroupSelectWidget: "AntdGroupSelectWidget",
    antdRelatedSelectWidget: "AntdRelatedSelectWidget",
  },
  number: {
    text: "TextWidget",
    select: "SelectWidget",
    ...
    antdSelect: "AntdSelectWidget",
    antdDisplay: "AntdDisplayWidget",
    antdNumberWidget: "AntdNumberWidget",
  },
  integer: {
    text: "TextWidget",
    select: "SelectWidget",
...
export function isCheckbox(_schema, definitions = {}) {
  const schema = retrieveSchema(_schema, definitions);
  const altSchemas = schema.oneOf || schema.anyOf;
...
export function isGroupCheckbox(schema, definitions = {}) {
  return isCheckbox(schema.items, definitions);
}
...
export function getFieldProps(uiSchema, jsonSchema) {
  // console.log('jsonSchema', jsonSchema)
  const label =
    uiSchema["ui:title"] || jsonSchema.title;
  const description =
    uiSchema["ui:description"] ||
    jsonSchema.description;
  const help = uiSchema["ui:help"] || jsonSchema.help;
...
export const transformErrors = (errors) => {
  return errors.map(error => {
    ...
    let eName = error.name
    switch (eName) {
      case 'maxLength':
        error.message = `最多${error.argument}个字符`
        break;
      case 'minLength':
        error.message = `最少${error.argument}个字符`
        break;
      case 'required':
        error.message = "不能为空"
        break;
      case 'minimum':
        error.message = `不能小于${error.argument}`
        break;
      case 'maximum':
        error.message = `不能大于${error.argument}`
        break;
      case 'pattern':
        error.message = `正则校验失败`
        break;
      default:
        error.message = "未知错误类型" + eName
        break;
    }
    return error;
  });
